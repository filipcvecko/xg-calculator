#!/usr/bin/env node
// Usage: node --env-file=.env scripts/backtest-control.js

import { calcOverUnder, dynamicRho } from '../src/math.js'
...
if (!KEY) {
  console.error('Missing VITE_FOOTYSTATS_KEY — run with: node --env-file=.env scripts/backtest-control.js')
  process.exit(1)
}
...
async function main() {
  ...
  for (const league of coefData.leagues) {
    ...
    const scoring_coef = 1 // control: all coefs forced to 1.0
    ...
    for (const m of completed) {
      ...
      // λ = geometric mean of attack × opponent defence, scaled by XG_SCALER
      let lH = Math.sqrt(homeScoredH * awayConcededA) * XG_SCALER
      let lA = Math.sqrt(awayScoredA * homeConcededH) * XG_SCALER
  ...
  writeFileSync('./backtest-control.json', JSON.stringify(output, null, 2))
  ...
  console.log(`  Saved to backtest-control.json`)
}
